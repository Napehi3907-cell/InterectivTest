...
$lesson_title = "Выбор ПОЛЬЗОВАТЕЛЯ";
if(isset($_GET['id_поль'])) {
    $lesson_id = (int)$_GET['id_поль'];
    $sql = "SELECT ФИО FROM PL WHERE id_поль = ?";
    $stmt = sqlsrv_prepare($link, $sql, array($lesson_id));

    if(sqlsrv_execute($stmt)) {
        $lesson = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
        if($lesson) {
            $lesson_title = htmlspecialchars($lesson['ФИО']);
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['view_report'])) {
    ...
    $stmt_course_name = sqlsrv_prepare($link, $sql_course_name);
    ...
    <header>
        ...
        if(isset($_GET['id_поль'])) {
            ...
            if(sqlsrv_execute($stmt)) {
                ...
                if($lesson) {
                    echo '<span> Добро пожаловать,' . htmlspecialchars($lesson['ФИО']) . '!</span>';
            ...
            echo '<div class="alert alert-warning">Приветствую, преподователь</div>';
        }
        ?>

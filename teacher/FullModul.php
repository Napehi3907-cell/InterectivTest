...
SELECT
    ...
    p.номер_телефона
FROM Курсы c
LEFT JOIN Преподаватели p ON c.id_преподавателя = p.id_преподавателя;
";
...
<body class="container">
    <header>
        <div class="nav-bar">
            <span>Полный каталог модулей</span>
                ...
                <ol class="courses-list">
                    <?php foreach ($courses as $course): ?>
                        <li class="course-item">
                            ...
                            <button type="button" class="btn" data-id="<?php echo $course['id_курса']; ?>">
    Добавить в временные модули
</button>
                        </li>
                    <?php endforeach; ?>
                </ol>
    ...
    <script>
        document.querySelectorAll('.course-item .btn[data-id]').forEach(button => {
    button.addEventListener('click', () => {
        const courseId = button.dataset.id;
        
        fetch('add_temp_course.php', {
            method: 'POST',
        ...
        .then(data => {
            if (data.success) {
                alert('Курс успешно добавлен во временные модули!');
                // Здесь можно добавить обновление списка временных курсов без перезагрузки страницы
            } else {
                alert(data.message);
            }
        })
